...
2.6.1 统一了包版本、中文文档和插件发现摘要；21 个生产工具的行为与已验证的 2.6.0 实现一致。2.6.0 为 `pcb_execute_plan` 增加 `build` 和只读 `reconcile`，在整批写入前阻断已建模的铜距、独立孔距及跨对象焊盘冲突，并通过 `executionLedger`、剩余计划和 `groupQuality` 统计支持功能组布局、局部试布与可靠续作。
...
2.5.1 将新板框改为原点锚定：圆形板框仍以 `[0,0]` 为圆心；简单多边形只需包含一个显式 `[0,0]` 顶点，不要求几何中心或包围盒中心位于原点。轴对齐长方形可把 `[0,0]` 作为一个角点，使相邻两边分别落在 X、Y 轴上。执行期焊盘重叠守卫继续生效：不同组件焊盘即使同网也不得重叠，独立焊盘或过孔不得侵入元件焊盘。同面移动在写入前预检；换面或换层后按真实焊盘复核，发现重叠即验证回滚。布局/重布局未显式指定 `batchSize` 时，展开操作不超过 100 个就作为一个完整布局轮次执行，超过时按 100 个分批；普通布线仍默认 24。结果返回 `padOverlapGate`，只有 `CLEAR`（或无布局对象时的 `NOT_APPLICABLE`）才允许开始下一轮。该守卫不替代原生 DRC。4.1.60 的 0.1 mil 焊盘坐标量化兼容、可续取 DRC 与有界明细均保留。

- 精确识别当前工程、窗口和 PCB 文档，拒绝过期目标或旧状态断言。
- 读取元件、焊盘、网络、层、约束、图元和关联原理图状态。
- 验证并执行显式 PCB 计划，包括原点锚定的原生圆形或多边形板框、器件移动、走线、过孔、独立 PTH/NPTH、网络端子焊盘和铺铜边界；小型少元件板可一次提交完整布局，布局写入前后检查跨对象焊盘重叠。
