...
2.5.0 要求新建圆形板框以 `[0,0]` 为圆心、多边形板框以 `[0,0]` 为包围盒中心，并新增执行期焊盘重叠守卫：不同组件焊盘即使同网也不得重叠，独立焊盘或过孔不得侵入元件焊盘。同面移动在写入前预检；换面或换层后按真实焊盘复核，发现重叠即验证回滚元件并返回 `PAD_OVERLAP_BLOCKED`。该守卫不替代原生 DRC。4.1.60 的 0.1 mil 焊盘坐标量化兼容、可续取 DRC 与有界明细均保留。

- 精确识别当前工程、窗口和 PCB 文档，拒绝过期目标或旧状态断言。
- 读取元件、焊盘、网络、层、约束、图元和关联原理图状态。
- 验证并执行显式 PCB 计划，包括原点居中的原生圆形或多边形板框、器件移动、走线、过孔、独立 PTH/NPTH、网络端子焊盘和铺铜边界；布局写入前后检查跨对象焊盘重叠。
