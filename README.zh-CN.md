...
2.4.7 新增两阶段受保护的元件清理工具：先生成预览与 Guard，再按同一状态批量解锁元件并只删除挂在元件上的 `Designator` 属性；普通丝印文本、非位号属性、元件身份和几何必须独立读回为未改变。生产工具面因此由 20 个增加为 21 个，并解除 SDK 注册表测试对旧工作区目录的硬编码。
...
- 通过 `pcb_cleanup_components` 预检或执行元件批量解锁与位号属性删除，同时保护接口说明等普通丝印。
