...
2.4.8 修复真实客户端中的位号清理适配：EasyEDA 会把元件 `Designator` 保留为必需身份属性，即使通用删除接口返回成功也不会真正移除。因此工具现在通过同时关闭 `keyVisible` 与 `valueVisible` 删除位号丝印显示，并独立验证 Designator 的 ID/值、元件身份与几何、普通丝印文本和非位号属性均未改变。2.4.7 已引入两阶段 Guard、批量解锁与第 21 个生产工具。
