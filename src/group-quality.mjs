...
export function inspectGroupQuality(snapshot, { groups, referenceLayers = [], detailLimit = 100 } = {}) {
  if (!snapshot || !['mil', 'mm'].includes(snapshot.units)) throw new Error('Explicit snapshot units required');
  ...
  const scale = snapshot.units === 'mil' ? 0.0254 : 1, allIds = new Set(), byNet = new Map(), byPad = new Map();
  const stat = net => { if (!byNet.has(net)) byNet.set(net, { net, straightLengthMm: 0, arcLengthMm: Array.isArray(snapshot.arcs) ? 0 : null, lineCount: 0, arcCount: 0, viaCount: 0, drilledPadCount: 0, padCount: 0, layers: new Map(), items: [] }); return byNet.get(net); };
  const refs = new Map();
  ...
  for (const [kind, items] of [['line', snapshot.lines], ['arc', snapshot.arcs ?? []], ['via', snapshot.vias], ['pad', snapshot.pads], ['fill', snapshot.fills ?? []]]) {
    for (const item of items) {
      ...
      const s = stat(item.net); let lengthMm = 0;
      if (kind === 'line') {
        for (const key of ['startX', 'startY', 'endX', 'endY']) finite(item[key], `${item.primitiveId}.${key}`);
        lengthMm = Math.hypot(item.endX - item.startX, item.endY - item.startY) * scale;
        s.straightLengthMm += lengthMm; s.lineCount++;
      } else if (kind === 'arc') {
        const arc = describeArc(item); lengthMm = arc.radius * Math.abs(arc.sweep) * scale;
        s.arcLengthMm += lengthMm; s.arcCount++;
      ...
      if (kind === 'line' || kind === 'arc') {
        const usage = s.layers.get(item.layer) ?? { layer: item.layer, lineCount: 0, arcCount: 0, lengthMm: 0 };
        usage[kind === 'line' ? 'lineCount' : 'arcCount']++; usage.lengthMm += lengthMm; s.layers.set(item.layer, usage);
      }
      s.items.push({ ...item, metricKind: kind, lengthMm });
    }
  }
  const results = groups.map(group => {
    ...
    const nets = group.nets.map(net => {
      ...
      if (!s) return { net, state: 'NOT_OBSERVED_IN_SNAPSHOT', straightLengthMm: null, arcLengthMm: null, viaCount: null, connectivity: 'NOT_EVALUATED' };
      const { items, layers, ...data } = s;
      for (const item of items) {
        if (refs.has(item.layer) && refs.get(item.layer) !== net && ['line', 'arc', 'fill'].includes(item.metricKind)) occupation.push({ primitiveId: item.primitiveId, net, layer: item.layer, kind: item.metricKind, lengthMm: item.metricKind === 'fill' ? null : item.lengthMm });
        if (region) {
    ...
    const pairs = (group.pairs ?? []).map(pair => {
      ...
      if (pair.maxDistanceMm !== undefined && !(finite(pair.maxDistanceMm, 'maxDistanceMm') > 0)) throw new Error('Positive pair distance budget required');
      if (!a || !b) { reasons.push({ code: 'PAIR_ENDPOINT_UNAVAILABLE', pair: label }); return { name: label, state: 'ENDPOINT_UNAVAILABLE', distanceMm: null, fromPadId: pair.fromPadId, toPadId: pair.toPadId }; }
      const distanceMm = Math.hypot(finite(a.x, 'pad x') - finite(b.x, 'pad x'), finite(a.y, 'pad y') - finite(b.y, 'pad y')) * scale;
      ...
      if (pair.maxDistanceMm !== undefined && distanceMm > pair.maxDistanceMm) reasons.push({ code: 'PAIR_DISTANCE_BUDGET_EXCEEDED', pair: label, distanceMm, maximumMm: pair.maxDistanceMm });
      return { name: label, state: 'MEASURED', fromPadId: a.primitiveId, toPadId: b.primitiveId, fromNet: a.net ?? null, toNet: b.net ?? null, sameNet: Boolean(a.net) && a.net === b.net, distanceMm };
    });
    const observed = nets.filter(net => net.state === 'OBSERVED');
    const totals = { straightLengthMm: observed.reduce((sum, net) => sum + net.straightLengthMm, 0), arcLengthMm: Array.isArray(snapshot.arcs) ? observed.reduce((sum, net) => sum + net.arcLengthMm, 0) : null, viaCount: observed.reduce((sum, net) => sum + net.viaCount, 0), drilledPadCount: observed.reduce((sum, net) => sum + net.drilledPadCount, 0), observedNetCount: observed.length, requestedNetCount: group.nets.length };
