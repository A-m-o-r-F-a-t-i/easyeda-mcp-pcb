...
export const editSchema={target,units:z.enum(['mil','mm']).default('mil'),operations:z.array(operation).min(1).optional(),planPath:z.string().min(1).optional().describe('Optional JSON file containing this same request; use operations for normal work'),save:z.boolean().default(true),view:z.enum(['auto','local','board','none']).default('auto'),bridgeUrl:z.string().url().optional()};
export const catalog=()=>({schema:'easyeda-pcb-edit/v3',units:'mil by default; pass units="mm" explicitly for metric input',operations:z.toJSONSchema(operation),notes:['MCP-first: these are simplified native operations, not an automatic layout or router.','Coordinates and paths are supplied by the caller. No collision/angle/design gate is inserted.','Unset fields retain their current values. Batch size is an implementation detail.','Partial results report applied, failed, unknown and not executed actions without automatic replay.']});
