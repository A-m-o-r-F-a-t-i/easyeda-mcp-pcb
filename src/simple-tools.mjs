...
const viewFields={...common,outputPath:z.string().optional(),units:z.enum(['mil','mm']).default('mil'),region:region.optional(),side:z.enum(['top','bottom','both']).default('top'),fit:z.enum(['board','all']).default('board'),layers:z.array(z.number().int()).optional(),nets:z.array(z.string()).optional(),pinLabels:z.boolean().default(true)};
export function createSimpleRegistry(previous){
 ...
 add('pcb_status','Read current PCB identity, units and client version. A unique active PCB needs no target parameter.',common,async a=>{const {context}=await exact(a);return {...await executeBridgeCode(context.bridge,codeFor(simpleReadRuntime,{...a,target:context.target,kind:'status'})),mcpVersion:'3.0.1',editingContract:'easyeda-pcb-edit/v3'};});
 ...
 add('pcb_pick','Find exact native objects at a point or in a rectangle without changing selection.',{...pick.definition.inputSchema,...common,units:z.enum(['mil','mm']).default('mil')},async a=>{const {args}=await exact(a);return unwrap(await pick.handler(args));});
 add('pcb_execute_plan','Bulk editor for placement, explicit routes, copper, mechanics and text. Units default to mil; pass units="mm" for metric input. No guard or design gate. Large ordered batches return factual partial results and optional SVG.',editSchema,applyEdits,false);
 ...
 add('pcb_inspect_pinmap','Read selected or all components with actual pad numbers/nets, poses, footprint dimensions and same-side orientation maps. Default mil; pass units="mm" explicitly for metric output.',{...common,refs:z.array(z.string()).optional(),units:z.enum(['mil','mm']).default('mil'),angles:z.array(z.number().finite()).optional(),orientationCoordinates:z.boolean().optional()},readOverview);
 ...
 add('pcb_export','Export one native file or a manufacturing bundle using kind=[...]. Handles native calls and filenames; refuses accidental file overwrite. Pick-and-place coordinates default to mil.',{...common,kind:z.union([kinds,z.array(kinds).min(1)]),outputPath:z.string().optional(),outputDirectory:z.string().optional(),scope:z.enum(['project','document']).optional(),format:z.enum(['xlsx','csv']).optional(),unit:z.enum(['mil','mm']).optional().describe('Pick-and-place coordinate unit; defaults to mil'),netlistType:z.enum(['JLCEDA_PRO','EASYEDA_PRO','PADS','ALTIUM_DESIGNER','ALLEGRO']).optional()},async a=>{
