...
The production profile stays at **21 tools**. PCB design choices remain in the Skill; no automatic placement, path search, schematic review, or board-specific dimensions and assembly sides are added.

`pcb_execute_plan` supports `build`, `validate`, `prepare`, `execute`, and `reconcile`. `build` fills missing old-state fields, including fillMode and component affectedNets, from a stable native snapshot; existing assertions and caller-specified positions/routes remain untouched. Complete plans go straight through prepare/execute. All plan files require absolute paths.

`prepare` checks the complete candidate plan against live modeled copper before issuing a guard. `execute` consumes that check only for the exact unchanged plan and guarded generation/epoch/source hash, or checks again. A conflict anywhere in this preflight prevents every write in that invocation. Copper and independent-drill spacing use `constraints.minClearance` and `minHoleClearance` in plan units. Omitted values mean overlap-only, not native-rule approval. Same-net copper may join; independent drills and cross-component pad/via intrusion are not exempt. Unknown modeled geometry remains blocking. Side flips must not share a plan with copper depending on stale pad coordinates; actual post-flip pads and existing rollback protections remain required. Pours, complex shapes, region/net-specific rules, bodies and actual reference-plane continuity still require the appropriate native/visual checks.

All supported geometry, including pure-line suffixes, uses one collision-aware native adapter. Readback distinguishes native merged collinear coverage from missing segments without changing global tolerances. Save acknowledgement and final-state verification cannot silently discard already observed geometry. `executionLedger` distinguishes VERIFIED, APPLIED_NEEDS_RECONCILE, OUTCOME_UNKNOWN and NOT_EXECUTED, with a separate saveState; verified does not imply saved. The checkpoint is a continuation record, not an atomic transaction or PCB backup.

`reconcile` executes **no mutations**. Two stable, bounded read passes classify each operation as APPLIED, PENDING or CONFLICT. Only unambiguous PENDING work produces a remainingPlan, requiring a new prepare/execute; conflicts block that remainder. After a recorded timeout or oversized response, the current process halves the batch size for the independently observed remainder only. No unknown write is automatically replayed. A restarted process can still reconcile geometry but does not claim to retain its predecessor's failure record.

`pcb_audit_geometry(checks:["groupQuality"])` accepts explicit groups with exact-case nets, optional exact pad pairs/distance budgets, a region in snapshot units, and a via budget. `referenceLayers:[{layer:16,net:"GND"}]` reports non-reference copper occupancy. Results separate straight/arc centerline sums, Euclidean pad distances, Via objects and plated terminal pads. Missing nets/endpoints stay unknown. These are evidence for retaining or rearranging a group, not end-to-end routed length, completed connectivity, reference continuity, or a universal board score.

## Installation and verification

Version 2.6.0 retains the verified 0.1 mil transformed component-pad coordinate quantization used by EasyEDA Pro 4.1.60 while keeping pad number, net, layer, and larger pose drift strict. It also retains resumable native DRC and bounded detail pages.

## Origin-anchored outlines and pad collision guard

`outline.create` accepts a new circle only when its center is `[0,0]`. A new simple polygon must include `[0,0]` as an explicit vertex; its geometric and bounding-box centers may be elsewhere. An axis-aligned rectangle may therefore use `[0,0]` as one corner, with its two adjacent edges lying on the X and Y axes. A polygon replacement supplied to `polyline.modify` follows the same anchor rule, while an existing legacy outline can still receive a non-geometric property edit without being silently moved.

Before a component placement write, the executor enumerates live component pins, standalone pads and vias and checks their physical copper envelopes. Different components cannot place pads over one another, and an independent pad or via cannot be placed over a component pad, regardless of network equality. A same-side move is rejected before writing. For a side or layer change whose native transform cannot be safely predicted, the executor checks the actual pins after the write and restores the exact previous component pose when overlap is found. A completed placement plan returns `padOverlapGate`; another placement round is allowed only when its state is `CLEAR` (or `NOT_APPLICABLE`). A conflict returns `PAD_OVERLAP_BLOCKED` when no prefix changed, or `PARTIAL_SUCCESS` with `blockingCause:"PAD_OVERLAP_BLOCKED"` after a verified prefix. Both results require repairing the current round before proceeding. This guard supplements rather than replaces milestone native DRC.
...
| `pcb_status(include:["capabilities"])`, `pcb_pick` | Method discovery and native/typed geometric queries. Method existence is not a functional test. Capability output also states which high-risk SDK methods remain intentionally unexposed. |
...
| `pcb_execute_plan` (build/validate/prepare/execute/reconcile) | Explicit `easyeda-pcb-plan/v2` native circular or closed polygon outlines, standalone PTH/NPTH round or slot pads, networked terminal pads, component moves, lines, through-vias and pour boundaries with guarded modifications/deletions. Layout/relayout defaults to one coherent batch up to 100 expanded operations and returns `padOverlapGate`; partial failures preserve a verified prefix and return the remaining suffix. |
| `pcb_execute_text_plan` (validate/prepare/execute) | `easyeda-pcb-text-plan/v1` strings and attribute presentation. Attribute key/value changes are rejected; use separate functional labels. |
...
| `pcb_save_and_drc`, `pcb_verify_api_gates` | Native status/control and resumable verbose DRC. `COMPLETED` reports the full violation total and summaries with a bounded detail page; `RUNNING` returns `drcJobId` for `save=false` continuation. The read-only gate still requires visual, topology, current, SI, mechanical and manufacturing review. |
| `pcb_import_schematic_changes` (preflight/execute) | Associated source, stable PCB digest, guarded import and before/after readback. No selective native ECO preview. |
...
| `pcb_capture_snapshot` (diagnostics only) | All twelve supported primitive categories, component and standalone pads, netlist, layers and constraints, saved to a new local JSON with byte verification. |
| `pcb_compare_snapshots` (diagnostics only) | Complete-array field comparison with missing categories/metadata and sensitive attribute semantics disclosed. |
| `pcb_export` | Native EPRO plus bounded Gerber, pick-and-place, BOM, test-point, netlist and IPC-D-356A export to new local files. No overwrite, dialog, upload or order action. |
| `pcb_capture_inspection_view` | Exact-tab current-viewport PNG transfer without pan, zoom, tab activation or selection changes. The inspection variant may isolate currently enabled layers and returns success only after exact API-visible layer-state restoration. |
...
When a batch stops after independently verifying earlier operations, the error returns `confirmedPlanOperations`, `failedOperationId`, `remainingOperationIds` and `nextAction`. Independent readback is retained for the verified prefix; save success is reported separately. Read the failed live object, remove already confirmed IDs, prepare a new guard and execute only the remaining suffix. Never replay the original full plan.
