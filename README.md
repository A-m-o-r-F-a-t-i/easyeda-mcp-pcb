...
Typed PCB tools over a local EasyEDA Pro Bridge, exposed as a compact 20-tool production profile plus diagnostics and legacy compatibility profiles. The executor applies explicit design decisions, reads actual results and rejects stale targets or old-state assertions. Version 2.4.6 completes EasyEDA Pro 4.1.60 no-hole SMD state normalization by treating the client's native non-finite `holeRotation` sentinel as the semantic zero default only when the requested hole rotation is zero. Finite nonzero rotation, real drill data, network, layer, pad number, metallization and lock identity remain strict. Version 2.4.5 moved exact pad modify/delete old-state reads to canonical `pcb_PrimitivePad.getAll()` identities. Version 2.4.1 added true native circular board outlines: `outline.create` with `shape:"CIRCLE"`, `position` and `diameter` compiles to EasyEDA's `['CIRCLE', cx, cy, radius]` polygon source, verifies center and radius independently, and renders the same circle in inspection SVGs. A circular board must not be approximated with polygon vertices. Existing closed polygon outlines, standalone PTH/NPTH round or slot objects, networked terminal pads, `boardDelta`, machine-readable `workflowReceipt`, exact-operation `recoveryDirective` and resumable partial batches remain supported. The service does not expose automatic placement, path search, automatic routing, route clearing, order placement, arbitrary JavaScript or whole-rule-table overwrite.
