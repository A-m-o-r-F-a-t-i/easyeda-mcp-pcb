# EasyEDA PCB MCP 2.4.7

Typed PCB tools over a local EasyEDA Pro Bridge, exposed as a compact 21-tool production profile plus diagnostics and legacy compatibility profiles. The executor applies explicit design decisions, reads actual results and rejects stale targets or old-state assertions. Version 2.4.7 adds a guarded two-stage component cleanup operation that can unlock all components and remove only component `Designator` attributes while independently proving component geometry, ordinary strings and non-designator attributes unchanged. It also makes SDK registry tests portable across the aggregate repository, sibling clones and configured reference roots. Version 2.4.6 completed EasyEDA Pro 4.1.60 no-hole SMD state normalization by treating the client's native non-finite `holeRotation` sentinel as the semantic zero default only when the requested hole rotation is zero. Finite nonzero rotation, real drill data, network, layer, pad number, metallization and lock identity remain strict. True native circular board outlines, closed polygon outlines, standalone PTH/NPTH round or slot objects, networked terminal pads, `boardDelta`, machine-readable `workflowReceipt`, exact-operation `recoveryDirective` and resumable partial batches remain supported. The service does not expose automatic placement, path search, automatic routing, route clearing, order placement, arbitrary JavaScript or whole-rule-table overwrite.
...
| `pcb_cleanup_components` | Preflight or execute one guarded bulk cleanup that unlocks components and removes only attached `Designator` attributes. It preserves ordinary strings, non-designator attributes, component identity and geometry. |
